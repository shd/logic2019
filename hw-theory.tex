...
\alpha [x := \theta] = \left\{ \begin{array}{ll}
  ...
  (\psi [x := \beta]) \star (\varphi [x := \beta]), &\mbox{ если $\alpha\equiv\psi\star\varphi$ }\\
  \neg(\psi [x := \beta]), &\mbox{ если $\alpha\equiv\neg\psi$ }\\
  \forall y.(\psi [x := \beta]),  &\mbox{ если $\alpha\equiv\forall y.\psi$ }\\
  \exists y.(\psi [x := \beta]),  &\mbox{ если $\alpha\equiv\exists y.\psi$ }
       \end{array} \right.
$$
...
\item Покажите, что $CX (\alpha, x) \cap FV(\beta) = \varnothing$ тогда и только тогда,
когда $\beta$ свободна для подстановки вместо $x$ в $\alpha$;
...
\item Покажите на основании данных определений, что если $CX (\alpha, x) \cap FV(\beta) = \varnothing$, 
то при любом множестве $D$ и любых оценках символов и переменных 
$\llbracket \alpha[x:=\beta]\rightarrow\exists x.\alpha\rrbracket = \texttt{И}$. 
\end{enumerate}
